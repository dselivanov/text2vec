...
Imagine we want to build a topic model with [lda](https://cran.r-project.org/web/packages/lda/) package. We have a collection of movie reviews stored in multiple text files on disk. 
...
`text2vec` provides functions to **easily** work with files input.

You need to do only a few things: 

1. Construct iterator over the files with `ifiles` function.
1. Provide reader function to `ifiles`. `text2vec` don't know anything about underlying files. They can be in plain text or some binary format.
...
Now we will construct DTM in `lda_c` format (as required by `lda` package):
```{r}
...
Now we can easily fit `LDA` model using `lda::lda.collapsed.gibbs.sampler()` function:
```{r, eval=FALSE}
library(lda)
# prior for topics
alpha = 0.1
