...
First of all lets split out dataset into two parts - *train* and *test*. We will show how to perform data manipulations on *train* set and then apply exactly the same manipulations on the *test* set:
...
Note that we use eactly the same functions from preprosessing and tokenization. Also we reuse/use the same `vectorizer` - function which maps terms to indices.
...
```
As we can see, performance on the test data is roughy the same as we expect from cross-validation.
...
Note that the new DTM has many fewer columns than the original DTM. This usually leads to both accuracy improvement (because we removed "noise") and reduce of the training time.
...
Note that here we first time touched *model* object in `text2vec`. At this moment reader should remember several important things about text2vec models:

1. **Models are mutable** - once you pass model to `fit()` or `fit_transform()` function, model will be fitted (modifed by `fit` or `fit_tranform` function).
1. Once model is fitted, it can be applied to new data with `transform(new_data, fitted_model)` method call.

More detailed overview of models and models API will be available soon in a separate vignette.

Once we have tf-idf reweighted DTM matrix we can fit our linear classifier again:
...
dtm_train = create_dtm(it_train, bigram_vectorizer) %>% 
  # note that we are modifying existing model from previous snippet
  fit_transform(tfidf)
