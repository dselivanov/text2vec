...
```
Since we set restriction to PMI scrore to 0 we got a lot of garbage collocations like *"when_its"*. Fortunately we can manually prune them manually and continue training. Let's filter by some thresholds:
```{r}
...
```
```{r}
us_cities = wv["new_york", , drop = FALSE] - 
  wv["usa", , drop = FALSE] + 
  wv["france", , drop = FALSE]
cos_sim = sim2(x = wv, y = us_cities, method = "cosine", norm = "l2")
head(sort(cos_sim[,1], decreasing = TRUE), 5)
```

## Topic models with collocations

Incorporating collocations into topic models are straightforward - need just to create document-term matrix and pass it to LDA model.
```{r}
data("movie_review")
prep_fun = function(x) {
