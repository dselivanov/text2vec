...
**text2vec** is a package which main goal is to provide **efficient framework** with **concise API** for **text analysis** and **natural language processing (NLP)** in R. It is inspired by [gensim](http://radimrehurek.com/gensim/) - excellent python library for NLP.
...
- Some parts (GloVe training) are fully **parallelized** using excellent [RcppParallel](http://rcppcore.github.io/RcppParallel/) package. This means, **parallel features works on OS X, Linux, Windows and Solaris(x86) without any additional tuning/hacking/tricks**.
