...
1. After model is fitted, it can be applied to a new data with `fitted_model$transform(new_data)` method or equivalent S3 method: `transform(new_data, fitted_model)`.

You can find more detailed overview of models and models API in a [separate vignette](#FIXME).
