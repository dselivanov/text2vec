...
#' @param granularity \link{integer} -  granularity is useful for management of granularity of splits.
#' If you expect that computational time on each chunk of your data will be distributed nerarly uniformly,
#' granularity = 1 is good choice because of little overheads in syncronizing parallel processes.
...
split_vector <- function(vector, splits, granularity = 1) {
  ...
  knots = ceiling(seq.int(from = 1, to = length(vector) + 1, length.out = splits * granularity + 1))
